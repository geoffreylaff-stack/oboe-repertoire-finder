 ...
 * inside a single instruction: "2nd, 3rd and 4th doubling english horn" lists
 * three chairs taking one instrument, not three separate doublings, and
 * "…cor anglais; 2nd cor anglais used only in Scherzo" is one doubling with a
 * remark after it.
 */
 ...
 *
 * One aside can name more than one instrument. Ligeti's Le Grand Macabre has
 * "second doubling oboe d'amore, 3rd doubling cor anglais" — two doublings by
 * two different chairs — and his Chamber Concerto has "doubling English horn
 * and oboe d'amore", two instruments taken by the same player. Reading only
 * the first left both works unfindable by anyone searching for English horn.
 */
...
export function parseInstrumentation(text) {
  ...
  for (const rawSegment of splitSegments(normalised)) {
    ...
    for (const aside of asides) {
      ...
      for (const { key, player } of readDoublings(aside)) {
